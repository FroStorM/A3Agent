...
class GeneraticAgent:
    ...
    def reload_config(self):
        ...
        if len(llm_sessions) > 0:
            ...
            self.llmclient = LLMClientLogger(self.llmclient, os.path.join(temp_dir, 'model_calls.log'))
            # Reset llm_no to 0 after reload (new session list may differ in size)
            self.llm_no = 0
            self.llmclient.real_client.backend = self.llmclient.backends[0]
        else:
            self.llmclient = None

    def next_llm(self, n=-1):
        self.llm_no = ((self.llm_no + 1) if n < 0 else n) % len(self.llmclient.backends)
        self.llmclient.real_client.backend = self.llmclient.backends[self.llm_no]
