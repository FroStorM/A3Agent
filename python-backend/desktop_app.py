...
if __name__ == "__main__":
    ...
    if is_frozen:
        ...
        if os.environ.get("AI_AGENT") == "TRAE" or os.environ.get("TRAE_SANDBOX_CLI_PATH"):
            user_data_dir = "/tmp/A3Agent/ga_config"
        else:
            user_data_dir = os.path.join(base_dir, "ga_config")
        ...
        bundle_memory = os.path.join(base_dir, "ga_config", "memory")
        if not os.path.isdir(bundle_memory):
        ...
        bundle_assets = os.path.join(base_dir, "ga_config", "assets")
        if not os.path.isdir(bundle_assets):
        ...
        os.chdir(base_dir)
        os.environ["GA_WORKSPACE_ROOT"] = base_dir
        os.environ["GA_USER_DATA_DIR"] = str(ensure_dir(os.path.join(base_dir, "ga_config")))
