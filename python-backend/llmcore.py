...
def _default_workspace_dir():
    ...
    if isinstance(root, str) and root:
        os.makedirs(root, exist_ok=True)
        return root
    root = os.environ.get("GA_BASE_DIR") or os.path.abspath(os.path.dirname(__file__))
    os.makedirs(root, exist_ok=True)
    return root
...
def _load_mykeys():
    ...
    # 如果环境变量已设置，优先使用环境变量
    if env_user_data_dir and isinstance(env_user_data_dir, str):
        base = env_user_data_dir
        print(f"[mykeys DEBUG] Using GA_USER_DATA_DIR from environment: {base}")
    else:
        # 否则尝试便携模式（exe同目录）
        portable_base = None
    ...
    primary_path = resolve_mykey_path(base, prefer_existing=True)
    print(f"[mykeys DEBUG] Resolved path: {primary_path}, exists={os.path.exists(primary_path) if primary_path else False}")
    loaded = _load_from_path(primary_path)
    if loaded is not None:
        print(f"[mykeys] SUCCESS loaded from {primary_path}: keys={list(loaded.keys())}")
        return loaded

    bundled_candidates = [
        get_resource_path(os.path.join("ga_config", "mykey.json")),
        get_resource_path(os.path.join("ga_config", "mykey.py")),
    ]
    print(f"[mykeys DEBUG] Checking bundled ga_config candidates: {bundled_candidates}")
    for path in bundled_candidates:
        loaded = _load_from_path(path)
        if loaded is not None:
            print(f"[mykeys] SUCCESS loaded from bundled ga_config: {path}")
            return loaded

    print(f"[WARN] mykey config not found under ga_config. Expected base={base}")
    return {}

# 延迟加载mykeys，避免在模块导入时就加载（此时环境变量可能还没设置）
_mykeys_cache = None
...
class _MykeysProxy:
    def __getattr__(self, name):
        return get_mykeys().get(name)

    def __getitem__(self, key):
        return get_mykeys()[key]

    def __len__(self):
        return len(get_mykeys())

    def __iter__(self):
        return iter(get_mykeys())

    def __contains__(self, key):
        return key in get_mykeys()

    def __bool__(self):
        return bool(get_mykeys())

    def get(self, key, default=None):
        return get_mykeys().get(key, default)

    def keys(self):
        return get_mykeys().keys()

    def items(self):
        return get_mykeys().items()

    def values(self):
        return get_mykeys().values()
...
def get_proxy():
    ...
    if proxy is None:
        proxy = get_mykeys().get("proxy", 'http://127.0.0.1:2082')
