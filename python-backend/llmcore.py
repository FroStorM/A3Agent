...
def _load_mykeys():
    ...
    # 优先使用base目录下的mykey.json（确保读写一致）
    primary_path = os.path.join(base, "mykey.json")
    print(f"[mykeys DEBUG] Primary path: {primary_path}, exists={os.path.exists(primary_path)}")
    if os.path.exists(primary_path):
        try:
            with open(primary_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            print(f"[mykeys] SUCCESS loaded from primary path {primary_path}: keys={list(data.keys())}")
            return {k: v for k, v in data.items() if not k.startswith('_')}
        except Exception as e:
            print(f"[WARN] failed to load primary path {primary_path}: {e}")

    # 如果primary_path不存在，尝试其他候选路径（向后兼容）
    candidates = [
    ...
    print(f"[mykeys DEBUG] Checking fallback candidates: {candidates}")
    for p in candidates:
        ...
        if os.path.exists(p):
            try:
                with open(p, "r", encoding="utf-8") as f:
                    data = json.load(f)
                print(f"[mykeys] SUCCESS loaded from fallback {p}: keys={list(data.keys())}")
                return {k: v for k, v in data.items() if not k.startswith('_')}
            except Exception as e:
    ...
    print("[WARN] mykey.json not found. Will use defaults.")
    return {}
