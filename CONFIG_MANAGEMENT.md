...
```
~/Library/Application Support/A3Agent/
└── workspace/
    └── ga_config/
        ├── mykey.json          # API密钥配置
        ├── memory/             # 记忆文件
...
- 自动从应用内置的默认配置复制到 workspace
- 输出日志：`[Launch] Ensuring ga_config at ...`
...
1. 用户下载 `A3Agent.app` 或 `A3Agent.dmg`
2. 双击运行
