...
fn exit_app(app: tauri::AppHandle) {
    ...
    if let Ok(guard) = app.state::<BackendPid>().0.lock() {
        if let Some(pid) = *guard {
            log(&format!("Terminating python-backend process tree (pid={})", pid));
            terminate_backend_process_tree(pid);
        }
    }
...
fn terminate_backend_process_tree(pid: u32) {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(src) = ga_config_src_dir.as_deref() {
                let targets = [ud_dir.to_path_buf()];
