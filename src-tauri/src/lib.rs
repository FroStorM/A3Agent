...
fn exit_app(app: tauri::AppHandle) {
    ...
    // 获取PID并直接终止进程
    if let Ok(guard) = app.state::<BackendPid>().0.lock() {
        if let Some(pid) = *guard {
            log(&format!("Terminating python-backend process (pid={})", pid));
...
fn kill_process_by_pid(pid: u32) {
    ...
    std::thread::spawn(move || {
        #[cfg(target_os = "windows")]
        {
            ...
            match result {
                Ok(output) => {
                    log(&format!("taskkill result: {}", String::from_utf8_lossy(&output.stdout)));
                }
                Err(e) => {
                    log(&format!("taskkill failed: {}", e));
                }
            }
        }
        #[cfg(not(target_os = "windows"))]
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Some(src) = ga_config_src_dir.as_deref() {
                // 只复制到 user_data_dir（即 workspace/ga_config），不复制到 workspace 根目录
                let _ = copy_missing_tree(src, ud_dir);
            }
        ...
        .run(|app_handle, event| {
            if let tauri::RunEvent::Exit = event {
                log("RunEvent::Exit triggered");
                if let Ok(guard) = app_handle.state::<BackendPid>().0.lock() {
                    if let Some(pid) = *guard {
                        log(&format!("Killing python-backend on exit (pid={})", pid));
